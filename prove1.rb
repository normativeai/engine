...
parser = Grammar.build do
  ...
  rule(:alls)         { (one "a") >> (one "l") >> (one "l") >> whitespace }
  rule(:exs)          { (one "e") >> (one "x") >> whitespace }
  ...
  rule(:equivs)       { whitespace <= (one "<") >> (one "=") >> (one ">") >= whitespace }
  ...
  rule(:atom)         { (rule :predicate) | (rule :const) }
  rule(:const)        { regex(/[a-z][A-Za-z\d]*/) }
  rule(:predicate)    { regex(/[a-z][A-Za-z\d]*/) >> (rule :popen) >> (rule :arg_list) >> (rule :pclose) }
  rule(:arg_list)     { ((rule :individual) >> (rule :comma) >> (rule :arg_list)) | (rule :individual)  }
  rule(:variable)     { regex(/[A-Z][A-Za-z\d]*/) }
  rule(:individual)   { (rule :variable) | (rule :const) }
  rule(:unary)        { (rule :popen) >> ( (rule :quant) | (rule :neg) | (rule :permitted) | (rule :ought) | (rule :o1) | (rule :pb) | (rule :nb) | (rule :nec)) >> (rule :pclose)  }
  rule(:quant)        { ( (rule :alls) | (rule :exs) ) >> (rule :variable) >> (one ":") >> whitespace >> (rule :formula) }
  ...
  rule(:binary)       { (rule :popen) >> ((rule :or) | (rule :and) | (rule :imp) | (rule :equiv)) >> (rule :pclose)  }
  ...
  rule(:imp)          { seq (rule :formula), (rule :imps), (rule :formula),
                        lambda {|f1,o,f2| f1 + ["=>"] + f2} }
  rule(:equiv)        { seq (rule :formula), (rule :equivs), (rule :formula),
                        lambda {|f1,o,f2| f1 + ["<=>"] + f2} }
